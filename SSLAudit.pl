...
sub check_dates {
	...
	my $days_left         = sprintf "%d",
	  ( ( $date_after_epoch - $date_now_epoch ) / ( 60 * 60 * 24 ) );
	...
	if ( $date_before_epoch > $date_now_epoch ) {
		...
		print "WARNING: Certificate will expire in $days_left days.\n";
	} else {
		print "Certificate date valid (will expire in $days_left days).\n";
	}
}
	...
	if ( !check_hostname( $host, $port ) ) {
		...
		foreach $protocol (@protocols) {
			...
			if ( @cipher_scores > 0 ) {
				...
				# No cipher scores, protocol not supported
				print "\n\n";
